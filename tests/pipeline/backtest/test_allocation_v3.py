...
def test_v2_text_applies_text_signal_adjustment() -> None:
    cfg = BacktestConfig.from_preset("v2_text", start_date=date(2012, 1, 3), end_date=date(2024, 6, 3))
    row = pd.Series(
        {
            "long_phase": "EXPANSION",
            "mid_regime": "NEUTRAL",
            "text_signal_state": "RISK_ON",
            "risk_gate": True,
            "run_universe": True,
        }
    )
    result = dispatch_allocation("v2_text", 0.68, row, cfg)
    assert result["action"] == "INCREASE"


def test_v2_text_respects_hard_gate_for_risk_on_overlay() -> None:
    cfg = BacktestConfig.from_preset("v2_text", start_date=date(2012, 1, 3), end_date=date(2024, 6, 3))
    row = pd.Series(
        {
    ...
    result = dispatch_allocation("v2_text", 0.05, row, cfg)
    assert result["action"] == "HOLD"


def test_v2_text_riskoff_ignores_risk_on_signal() -> None:
    cfg = BacktestConfig.from_preset("v2_text_riskoff", start_date=date(2012, 1, 3), end_date=date(2024, 6, 3))
    row = pd.Series(
        {
            "long_phase": "EXPANSION",
            "mid_regime": "NEUTRAL",
            "text_signal_state": "RISK_ON",
            "text_signal_confidence": 0.9,
            "risk_gate": True,
            "run_universe": True,
        }
    )
    result = dispatch_allocation("v2_text_riskoff", 0.68, row, cfg)
    assert result["action"] == "HOLD"


def test_v2_text_riskoff_guarded_requires_confidence_threshold() -> None:
    cfg = BacktestConfig.from_preset("v2_text_riskoff_guarded", start_date=date(2012, 1, 3), end_date=date(2024, 6, 3))
    row = pd.Series(
        {
            "long_phase": "EXPANSION",
            "mid_regime": "NEUTRAL",
            "text_signal_state": "RISK_OFF",
            "text_signal_confidence": 0.58,
            "risk_gate": True,
            "run_universe": True,
        }
    )
    result = dispatch_allocation("v2_text_riskoff_guarded", 0.72, row, cfg)
    assert result["action"] == "HOLD"


def test_v2_text_riskoff_guarded_applies_small_defensive_adjustment() -> None:
    cfg = BacktestConfig.from_preset("v2_text_riskoff_guarded", start_date=date(2012, 1, 3), end_date=date(2024, 6, 3))
    row = pd.Series(
        {
            "long_phase": "EXPANSION",
            "mid_regime": "NEUTRAL",
            "text_signal_state": "RISK_OFF",
            "text_signal_confidence": 0.75,
            "risk_gate": True,
            "run_universe": True,
        }
    )
    result = dispatch_allocation("v2_text_riskoff_guarded", 0.75, row, cfg)
