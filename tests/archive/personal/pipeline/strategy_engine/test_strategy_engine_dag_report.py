...
def test_generate_llm_analysis_fail_open_on_error(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "pretrend.observability.explainability.context._get_report_ollama_client",
        _boom,
    )
    result = _generate_llm_analysis(
        {"dummy": "payload"},
...
def test_generate_llm_analysis_gemini_success(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "pretrend.observability.explainability.context._call_gemini",
...
def test_generate_llm_analysis_uses_analyzer_first(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "pretrend.observability.explainability.context.generate_report_via_analyzer",
