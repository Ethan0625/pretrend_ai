...
import { useMacroExplain, useMeta, useRegimeExplain, useSimilarityExplain } from "@/api/hooks";
...
export function Explain() {
  ...
  const similarity = useSimilarityExplain(activeDate, view);
  const macro = useMacroExplain(activeDate);

  return (
    <>
      <Toolbar tradeDate={activeDate} view={view} onTradeDate={setTradeDate} onView={setView} onRefresh={() => {
        regime.refetch();
        similarity.refetch();
        macro.refetch();
      }} />
      <div className="grid-3">
        ...
        <ExplainCard title="유사도 설명" endpoint={`/api/v1/similarity/explain?query_date=${activeDate}&view=${view}`} result={similarity} />
