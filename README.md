...
* 🧭 **Risk-Control 전략 문서 구조(v0)**
  ...
  * 전략 흐름: `Layer -> Market Structure(4축) -> Composer -> Universe -> Allocation Engine -> Weekly Report`
  * 상태 기반 Allocation 중심으로 문서/계약 구조 재정의
  * v0는 총 투자 비율(`invested_ratio`) 조절만 수행, Universe 내부 가중치 조절은 제외
...
Gold Macro / Gold EOD Snapshot
        ↓
Strategy Engine (Axis×Horizon → Policy → Universe → Allocation → Sell)
        ↓
