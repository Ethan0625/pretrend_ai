...
### A3) Strategy Engine
Strategy Engine은 4개 axis 입력(`macro_policy`, `price_volatility`, `flow_structure`, `sentiment`)을 Horizon 엔진에서 집약해 `long/mid/short` 3개 상태를 산출한다. Axis×Horizon 의존성 매트릭스는 계약으로 유지하되, 현재 저장 출력은 12개 독립 슬롯이 아니라 3-state 집약 결과다. 엔진 순서는 Axis Feature Builder, Axis×Horizon State, Market Position, Policy Selector, Universe-ETF Selector, Allocation Engine, Sell Advisor, Next Step Signal, Weekly Report로 고정한다. Market Position은 상태 벡터를 표준화해 정책 선택과 실행 경계 출력의 공통 입력을 제공한다. Policy Selector는 registry에서 정책을 선택하되 점수 최적화 없이 상태 기반 룰만 사용한다. Universe-ETF Selector는 무엇을 보유할지 결정하고 Allocation Engine은 총 투자 비율만 조정한다. Sell Advisor는 v0에서 매도 예산과 우선순위만 정의하며 종목별 정밀 매도 비율은 다루지 않는다. Next Step Signal은 운용 게이트 입력(snapshot)으로 사용되며 soft gate 강도 조절에만 사용된다. 모든 출력은 `decision_date` 스냅샷으로 저장된다.
...
flowchart TD
    ...
    AFB --> AHS[Axis x Horizon State (3 states + details)]
...
### E2) 디렉터리 구조 예시
- `data/strategy/axis_horizon_state/decision_date=YYYY-MM-DD/axis_horizon_state_YYYYMMDD.parquet`
...
- `data/strategy/next_step_signal/decision_date=YYYY-MM-DD/next_step_signal_YYYYMMDD.parquet`
- `data/strategy/next_step_history/year=YYYY/month=MM/*.parquet` (precompute history, key=`trade_date+decision_date_ref`)
...
- Axis×Horizon 출력은 `long/mid/short` 3-state로 고정한다.
- `axis_horizon_state` 스냅샷은 상태값과 함께 근거 컬럼(`long_detail_json`, `mid_detail_json`, `short_detail_json`)을 저장한다.
- 상태 출력은 ENUM 기반만 허용하며 numeric score 출력은 금지한다.
- 용어 사전(표시 기준):
  - `mid_regime`: 중기 성향 라벨 (`RISK_ON/NEUTRAL/RISK_OFF`)
  - `risk_gate`: 단기 공황 차단 게이트 (`False`면 PANIC)
...
- Axis×Horizon 3-state + detail 컬럼 존재 검증
...
- 실행 단계:
  1. Axis Features (4축)
  2. Axis×Horizon State (3-state 집약 + detail)
  3. Market Position
