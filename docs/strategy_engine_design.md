...
### A3) Strategy Engine
Strategy Engine은 `Axis × Horizon = 12 slots` 상태 해석 구조를 사용한다. Axis는 `macro_policy`, `price_volatility`, `flow_structure`, `sentiment` 4개 근거 축이며 Horizon은 `long`, `mid`, `short` 관측 해상도다. 엔진 순서는 Axis Feature Builder, Axis×Horizon State, Market Position, Policy Selector, Universe Selector, Allocation Engine, Sell Planner, Weekly Report로 고정한다. Market Position은 상태 벡터를 표준화해 정책 선택과 실행 경계 출력의 공통 입력을 제공한다. Policy Selector는 registry에서 정책을 선택하되 점수 최적화 없이 상태 기반 룰만 사용한다. Universe Selector는 무엇을 보유할지 결정하고 Allocation Engine은 총 투자 비율만 조정한다. Sell Planner는 v0에서 매도 예산과 우선순위만 정의하며 종목별 정밀 매도 비율은 다루지 않는다. 모든 출력은 `decision_date` 스냅샷으로 저장된다.
...
flowchart TD
    ...
    PS --> US[Universe Selector]
    PS --> AE[Allocation Engine]
    US --> WTH[WHAT_TO_HOLD]
    AE --> EXP[HOW_MUCH_EXPOSURE]
    PS --> SP[Sell Planner]
    SP --> SELL[HOW_MUCH_TO_SELL]
...
| Flow Structure Proxy (v0 Partial) | `data/gold/eod/eod_features/...` + 파생 집계 | Grain `(trade_date)` 또는 `(symbol, trade_date)` | `volume`, breadth proxy(`iwm_spy_ratio`) | `volume_zscore_20d`, `obv_*` | 부분 수집 허용; 누락 시 `flow_structure=UNKNOWN` |
...
## SECTION J — Text Signals & LLM(AI Agent) Integration Port (v1+ Reserved)

### 목적
- Text/LLM 입력은 Strategy Engine 내부 수집이 아니라 외부 Text Pipeline/Agent 레이어에서 생성된 신호를 옵션 포트로 소비한다.
- v0 동작은 Text 입력 없이도 유지되어야 한다.
