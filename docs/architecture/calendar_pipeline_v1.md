...
- **Bronze**
  ...
  - Columns:
    ...
    - actual_value FLOAT (없으면 NULL)
    - source TEXT (예: investing_com, bls_gov, manual_csv)
    - run_id TEXT
...
| Econ events 데이터 소스 | TBD | curated CSV / external API / manual 입력 결정 필요 |
| 파일 위치(예정) | N/A | `src/pretrend/pipeline/calendar/*.py`, `tests/pipeline/test_calendar.py` |

---
- Calendar Pipeline v1은 **설계 계약**이며, 구현 전까지 Gold 실행을 막는다.
