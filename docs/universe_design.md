...
**Version:** 2026.02.12  
**Purpose:** Risk-Control 전략 구조에서 Universe v1의 역할/입력/출력 정의

---

## 1. Overview

본 문서는 Universe v1의 설계 목적과 동작 경계를 정의한다.
Universe는 독립 전략 엔진이 아니라,
...
## 2. Universe Pipeline Structure (v1)
...
핵심 원칙:
- Universe는 Composer 출력만 의존한다.
- Universe는 Observability 라벨을 변경하지 않는다.
- Allocation은 Universe 결과를 소비하되, v0에서 총 투자비율만 조절한다.
...
- Universe는 Composer 하위 모듈(long/mid/short)을 직접 참조하지 않는다.
- v0에서 Universe는 비중 배분을 결정하지 않는다.
...
- Layer:
  - Gold Macro/Gold EOD가 Universe 입력의 상위 데이터 소스다.
- Allocation Engine v0:
  - Universe 후보 리스트를 소비한다.
  - `risk_gate=false`일 때는 증가 차단 규칙을 적용한다.
  - 총 투자비율 조절만 수행하며, Universe 내부 비중은 조절하지 않는다.

---

## 9. Roadmap (Universe 관점)
