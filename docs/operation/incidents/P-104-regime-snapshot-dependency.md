...
- P34에서 `src/pretrend/observability/regime/regime_feature_builder.py`를 추가했다.
- `similarity_build_dag`의 `build_market_state_features` task를 Gold DB 기반 `build_market_state_similarity_features_from_db()` 호출로 교체했다.
- 기존 `data/strategy` snapshot 경로는 호환용 deprecated 함수로만 유지하고, snapshot 관련 import는 함수 내부 lazy import로 내렸다.

---

## 7. 검증

- `/api/v1/meta` 및 운영 SQL 기준:
...
- P34 운영 smoke:
  - Airflow 컨테이너에서 `build_market_state_similarity_features_from_db(2026-05-26, 2026-05-26)` 실행 → `gold_market_state_similarity_feature` 1 row upsert
  - 이어서 `build_similarity_regime(2026-05-26, 2026-05-26)` 실행 → `similarity_regime` 100 row upsert
- 테스트:
  - `conda run -n pretrend_pytest pytest --gate fast -q --tb=short` → `537 passed, 27 skipped, 12 deselected`
- 대표 API smoke:
...
- Observability 전용 regime feature builder가 Gold DB에서 직접 `market_state`/`rotation` source를 재구성한다.
- `similarity_build_dag`는 legacy strategy snapshot 없이 `gold_market_state_similarity_feature`를 생성한다.
- `tests/observability/regime/test_regime_feature_builder.py`, `tests/observability/similarity/test_market_state_feature_runtime_source.py`, `tests/dags/test_similarity_build_dag.py`가 builder range trim, lazy import, DAG source contract를 고정한다.

---

## 9. 남은 부채

- 최신 운영일 기준 replay API smoke는 P35에서 확인한다.
- `data/strategy/*` path 의존은 deprecated 호환 함수에만 남아 있다. 신규 DAG 경로에서는 사용하지 않는다.

---

## 10. 메모

P34 이후 dashboard read-only 관측 계층의 regime similarity 갱신 경로는 legacy strategy execution snapshot에 의존하지 않는다.
